...
async fn test_coin_reservation_validation() {
    ...
    {
        let coin_reservation = test_env.encode_coin_reservation(sender1, 3, 10000000);

        let tx = TestTransactionBuilder::new(sender1, coin_reservation, test_env.rgp)
    ...
    {
        let coin_reservation = test_env.encode_coin_reservation(sender1, 0, 0);
    ...
    {
        let coin_reservation = test_env.encode_coin_reservation(sender1, 0, 100);
...
async fn test_valid_coin_reservation_transfers() {
    ...
    let recipient_balance = test_env.cluster.get_sui_balance(recipient).await;
...
async fn test_valid_coin_reservation_gas_payments() {
    ...
    let initial_sender_balance = test_env.cluster.get_sui_balance(sender).await;
    ...
    let final_sender_balance = test_env.cluster.get_sui_balance(sender).await;
    assert_eq!(
        final_sender_balance.total_balance,
        initial_sender_balance.total_balance - gas_charge - 1
    );
}
