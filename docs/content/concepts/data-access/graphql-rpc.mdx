...
The scope optionally provides a **root object** bound which only applies to queries that fetch dynamic fields. This fetches the dynamic fields as they were at the end of a specific checkpoint or when their root object was at some given version. For any wrapped or child (object-owned) object, its root object is defined recursively as:

- the root object of the object it is wrapped in, if it is wrapped,
- the root object of its owner, if it is owned by another object, otherwise
- the object itself.

If a dynamic field's root object has version `v`, its own version, `w` is the latest version such that `w <= v` -- that is, the latest version of the dynamic field that existed when its root object was at version `v`.
...
- live object set queries are not available under executed transaction scopes, or if there is a root object bound given in terms of a specific version, because these queries rely on data that is indexed at the checkpoint level.
- queries that paginate through history are not available under executed transaction scopes, because it's not possible ot pin-point where in the history the transaction will fall before it has been indexed.

### Setting Checkpoint Scope

Queries can be run against a historical checkpoint either using `Checkpoint.query`, or in the case of a [consistent](#consistency) live object set query (like fetching an address' owned objects or balances, or an object's dynamic fields), by continuing to paginate using a cursor obtained from a previous query at that checkpoint:
...
`AtCheckpoint` will return the last 5 transactions to execute as of the end of the checkpoint with sequence number `$cp`, while `NextBalancesPage` will fetch the next 10 balances for `$address` after the page that ended at cursor `$after`. It is also possible to combine both approaches, to ensure that the first page of a live object query is fetched from a specific checkpoint:
...
Responses to these queries are subject to [retention](#retention). If `$cp` is outside the retention window `Query.transactions`, `AtCheckpoint` will return no results, while an attempt to continue paginating `NextBalancesPage` at a checkpoint outside the consistent range will return an error.
...
This property applies recursively to nested dynamic field queries, and also applies if the root object is fetched at a specific checkpoint using the `atCheckpoint` parameter or at the latest checkpoint (by omitting all parameters), in which case the root object bound is checkpoint-based.

The `rootVersion` parameter overrides this implicit bound, fetching the object as if it was subject to root version bound governed by the parameter, which also applies to queries nested underneath it. This is necessary when fetching a dynamic field directly (as an object), as an object's version is not updated when its children are updated unless it is a root object.

In the following query, `$id` is the address of a dynamic field fetched as it existed when its root object was at version `$r`, and the nested query fetches a nested dynamic field with name `42u64` owned by a wrapped object stored in field `foo` of the dynamic field. The nested fetch is subject to the same root object version bound, `$r`.
...
While nested within a scope that has a root object bound, it is possible to reset or override the bound, using the `objectAt` or `addressAt` queries. These fields query the same entity but at a different position in its history. If these fields are not provided any parameters, they fetch the state of the object at the latest known checkpoint to the GraphQL service. The following query fetches the latest version of an object that was previously fetched at version `$v`:
